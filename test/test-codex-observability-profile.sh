...
grep -q '^environment = "kh"$' "$CONFIG" || { echo 'FAIL: Codex metrics environment must equal harness profile'; exit 1; }
...
grep -q '"obs.profile" = "kh"' "$CONFIG" || { echo 'FAIL: Codex span profile missing'; exit 1; }
