...
if [[ ${#mcp_json_files[@]} -gt 0 ]]; then
  python3 - "${mcp_json_files[@]}" >> "$tmp_config" <<'PY'
import json, re, sys
mcp_paths = sys.argv[1:]
servers = {}
owners = {}
...
sync_bundled_marketplace_from_app_bundle() {
  ...
  local src_manifest="$app_marketplace/plugins/computer-use/.codex-plugin/plugin.json"
  local dest_manifest="$dest/plugins/computer-use/.codex-plugin/plugin.json"
...
remove_legacy_browser_runtime_surface() {
  rm -rf "$CODEX_HOME/browser" "$CODEX_HOME/node_repl"
}
